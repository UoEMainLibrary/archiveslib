...
<select name="year">
<option value="">All</option>
<? 	 $sql_str="SELECT DISTINCT year FROM cms_accessions ORDER BY year ASC";
	 $yearsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
...
echo "<h3></h3>";
	 ...
	 $accsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");	 
	 $results = mysql_fetch_array($accsearch);
...
<form><input type=button value="Associate this with a collection" onClick="javascript:popUp('cms/popups/note.php?view=assoc&amp;id=<? echo $results['id'] ?>')"></form>


	 
<? 	 if (($results['acc_type']) == "MS") {
...
while ($results2 = mysql_fetch_array($boxsearch)):
...
endwhile;
echo "</table>";  ?>
<form><input type=button value="Manage locations" onClick="javascript:popUp('popups/popup.php?popup=locations&amp;id=<? echo $results['id'] ?>')"></form>
<?
}
...
<input name="year" type="hidden" value="<? echo $current_year ?>" />
</td></tr>
<?
...
$sql_str="SELECT * FROM cms_accessions WHERE acc_type LIKE '$acc_type' AND year='$current_year' ORDER BY accession DESC LIMIT 1";
	 $accsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
	 $result = mysql_fetch_array($accsearch);
	 $next_acc = $result['accession'] + 1;

?>
<tr><td class='label'>Accession</td><td><? echo $next_acc ?><input name="accession" type="hidden" value="<? echo $next_acc ?>" /></td></tr>
<tr><td class='label'>Accession Type</td><td><? echo $acc_type ?></td></tr>
...
<input name="func" type="hidden" value="accessions" />
<input name="acc_type" type="hidden" value="<? echo $acc_type ?>" />
<input name="" type="submit" value="Continue" />
</form>
...
$vw = "add";
 ...
 ?>
<input name="view" type="hidden" value="<? echo $vw; ?>" />
<input name="year" type="hidden" value="<? echo $current_year ?>" />
</td></tr>
<tr><td class='label'>Accession</td><td><? echo $_GET['accession'] ?><input name="accession" type="hidden" value="<? echo $_GET['accession'] ?>" /></td></tr>
<tr><td class='label'>Accession Type</td><td><? echo $acc_type ?></td></tr>
<tr><td class='label'>Description **</td><td><textarea name="description" cols="80" rows="3"><? echo $_GET['description'] ?></textarea></td></tr>
<tr><td class='label'>Creator</td><td><input name="creator" type="text" size="98" value="<? echo $_GET['creator'] ?>" /></td></tr>
<tr><td class='label'>Donor/Depositor **</td><td><textarea name="depositor" cols="80" rows="3"><? echo $_GET['depositor'] ?></textarea></td></tr>
<tr><td class='label'>Ownership</td><td>

<? if (($_GET['type']) == 'transfer') { $type_val="Transfer (internal)"; }
...
?>
<select name="type">
<option value="<? echo $_GET['type'] ?>"><? echo $type_val ?></option>
<? if ($acc_type == 'EUA') { ?>
<option value="transfer">Transfer (internal)</option>
<option value="donation">Donation (external)</option>
<option value="loan_i">Loan (internal)</option>
...
<tr><td class='label'>Conditions</td><td><textarea name="conditions" cols="80" rows="3"><? echo $_GET['conditions'] ?></textarea></td></tr>
<tr><td class='label'>Copyright</td><td><textarea name="copyright" cols="80" rows="3"><? echo $_GET['copyright'] ?></textarea></td></tr>
<tr><td class='label'>Access Restrictions</td><td><textarea name="restrictions" cols="80" rows="3"><? echo $_GET['restrictions'] ?></textarea></td></tr>
<tr><td class='label'>Extent</td><td><input name="extent" type="text" size="98" value="<? echo $_GET['extent'] ?>" /></td></tr>
<tr><td class='label'>Physical Condition</td><td><textarea name="phys_cond" cols="80" rows="3"><? echo $_GET['phys_cond'] ?></textarea></td></tr>
<tr><td class='label'>Comments</td><td><textarea name="comments" cols="80" rows="3"><? echo $_GET['comments'] ?></textarea></td></tr>
<tr><td class='label'>Documentation</td><td><textarea name="documentation" cols="80" rows="3"><? echo $_GET['documentation'] ?></textarea></td></tr>
<? if ($acc_type == 'RBP') { ?>
<tr><td class='label'>Shelfmark</td><td><input name="extent" type="text" size="98" value="<? echo $_GET['shelfmark'] ?>" /></td></tr>
<? } ?>
</table>
<input name="func" type="hidden" value="accessions" />
<input name="acc_type" type="hidden" value="<? echo $acc_type ?>" />
...
##  echo "<p>".$sql_str."</p>";
		mysql_query($sql_str) or die("Insert failed!<br />" . mysql_error());
