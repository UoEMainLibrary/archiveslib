...
elseif ($view == "viewtask") {
 ...
 $sql_str="SELECT * FROM collections WHERE coll=$coll";
   ...
   $results = mysql_fetch_array($collsearch);
	 ...
	 $tasksearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
 
echo "<table cellpadding='5' width='90%' class='detail'>";

$results = mysql_fetch_array($tasksearch);
...
echo "<h4>Notes</h4>";
  $sql_str="SELECT * FROM notes WHERE tablename LIKE 'tasks' AND tableid=$id";
	 $notesearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
...
while ($results = mysql_fetch_array($notesearch)):
...
## if editor display Delete Note option
  ...
  <input type=button value="Delete" onClick="javascript:popUp('cms/popups/note.php?view=delete&amp;id=<? echo $results['id'] ?>')">
  </form></div>
...
## if editor display Add Note option
  ...
  if ( $edit_permissions == "y" ) { ?>
    ...
    <form>
    <input type=button value="Add a Note" onClick="javascript:popUp('cms/popups/note.php?view=add&amp;tablename=tasks&amp;tableid=<? echo $id ?>')">
    </form>
  <? }
	## end Add Note loop
...
echo $sql_str;
		mysql_query($sql_str) or die("Insert failed!");
