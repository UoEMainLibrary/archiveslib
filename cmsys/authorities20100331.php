...
elseif ($_GET['type'] == "geog") { $sql_str="SELECT * FROM $table WHERE term LIKE '%$searchterm%' OR island LIKE '%$searchterm%' OR city LIKE '%$searchterm%' OR territory LIKE '%$searchterm%' OR county LIKE '%$searchterm%' OR country LIKE '%$searchterm%' AND suppress IS NULL ORDER BY term ASC, island ASC, city ASC, county ASC, country ASC"; }
...
##	 $sql_str="SELECT * FROM cms_auth_geog WHERE created_for LIKE'$created_for%' AND suppress IS NULL ORDER BY term ASC, island ASC, city ASC, county ASC, country ASC";
	 	 ...
	 	 $sql_str="SELECT CONCAT(term, island, city, county, country) AS geogterm, id, term, island, city, territory, county, country FROM cms_auth_geog WHERE created_for LIKE'$created_for%' AND suppress IS NULL ORDER BY geogterm ASC";
...
echo "<tr><td><a href='".$_SERVER['PHP_SELF']."?func=authorities&amp;view=geogview&amp;id=".$results['id']."'>".$results['term']." ".$results['island']." ".$results['city']." ".$results['territory']." ".$results['county']." ".$results['country']."</a></td><td><a href='".$_SERVER['PHP_SELF']."?func=authorities&amp;view=geogedit&amp;id=".$results['id']."'>Edit</a></td></tr>";
endwhile;
echo "</table>";
...
echo "<tr><td class='label'>Parish</td><td>".$results['territory']."</td></tr>";
...
<input type="hidden" name="id" value="<? echo $results['id'] ?>" />
<table summary="">
<tr><td class="label">Term / Area</td><td><input name="term" size="75" type="text" value="<? echo $results['term'] ?>" /></td></tr>
<tr><td class="label">Island</td><td><input name="island" size="75" type="text" value="<? echo $results['island'] ?>" /></td></tr>
<tr><td class="label">City</td><td><input name="city" size="75" type="text" value="<? echo $results['city'] ?>" /></td></tr>
<tr><td class="label">Parish</td><td><input name="territory" size="75" type="text" value="<? echo $results['territory'] ?>" /></td></tr>
<tr><td class="label">County</td><td><input name="county" size="75" type="text" value="<? echo $results['county'] ?>" /></td></tr>
<tr><td class="label">Country</td><td><input name="country" size="75" type="text" value="<? echo $results['country'] ?>" /></td></tr>
...
$term = mysql_real_escape_string($_GET['term']);
$island = mysql_real_escape_string($_GET['island']);
$city = mysql_real_escape_string($_GET['city']);
$territory = mysql_real_escape_string($_GET['territory']);
...
$sql_str="UPDATE cms_auth_geog SET id='$id', term='$term', island='$island', city='$city', territory='$teritory', county='$county', country='$country',  alt_form='$alt_form', alt_form_lang='$alt_form_lang',  use_for='$use_for', locator='$locator', source='$source', other='$other', ext_id='$ext_id', notes='$notes', last_edited_by='$last_edited_by' WHERE id='$id' LIMIT 1";
...
elseif ($view == "geogadd") {
	...
	## $sql_str="SELECT * FROM cms_auth_geog WHERE suppress IS NULL ORDER BY term ASC";
	 $sql_str="SELECT CONCAT(term, island, city, county, country) AS geogterm, id, term, island, city, county, country FROM cms_auth_geog WHERE created_for LIKE'$created_for%' AND suppress IS NULL ORDER BY geogterm ASC";
...
while ($results = mysql_fetch_array($geogsearch)):
echo $results['term']." ".$results['island']." ".$results['city']." ".$results['county']." ".$results['country'] ."\n";
endwhile;
echo "</textarea></td></tr></table>"; ?>
...
<tr><td class="label">Parish</td><td><input name="territory" size="75" type="text" /></td></tr>
...
$sql_str="INSERT INTO cms_auth_geog (id, term, island, city, territory, county, country, alt_form, alt_form_lang, use_for, locator, source, other, ext_id, notes, created_for, created_by, created_on) VALUES  (NULL, '$term', '$island', '$city', '$territory', '$county', '$country', '$alt_form', '$alt_form_lang', '$use_for', '$locator', '$source', '$other', '$ext_id', '$notes', '$created_for', '$created_by', '$created_on')";
...
echo "<p><a href='".$_SERVER['PHP_SELF']."?func=authorities&amp;view=geogview&amp;id=".$results['id']."'>".$results['term']." ".$results['island']." ".$results['city']." ".$results['territory']." ".$results['county']." ".$results['country']."</a> has been added</p>";
