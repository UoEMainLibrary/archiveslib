...
while ($results = mysql_fetch_array($accpagesearch)):
...
$sm_sql_str="SELECT * FROM cms_accessionsToShelfmarks WHERE year LIKE '$year' AND number LIKE  '$number' AND suffix LIKE '$suffix'";
	 $smsearch = mysql_db_query($dbname, $sm_sql_str, $id_link) or die("Search Failed!");
	 
	 echo "<h3>Shelfmarks</h3>";
	 
	 echo "<p>";

while ($smresults = mysql_fetch_array($smsearch)):
