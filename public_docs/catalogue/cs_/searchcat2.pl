...
my $www_root = "/home/archives/public_docs";
...
open(HTML_INPUT, "$www_root/includes/header.php");
while (<HTML_INPUT>) {
  print $_;
}
...
open(HTML_INPUT, "$www_root/includes/lowerheader_cat.php");
while (<HTML_INPUT>) {
  print $_;
}
...
if ((scalar @terms) > 5) {
  ...
  ##  print "<ol start=\"$startpoint\">\n";
    ...
    for (my $count = $startpoint; $count <= $endpoint; $count += 1) {
      ...
      print "<tr><td valign=\"top\" width=\"10\">$count</td><td valign=\"top\" width=\"65%\"><a href=\"$view_isad_url?id=$result&view=basic\">$titletext</a> ";
