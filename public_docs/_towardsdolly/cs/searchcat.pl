...
my $repository_root = "/home/archives/towardsdolly_catalogue_repository";
my $cat_root = "/home/archives/public_docs/towardsdolly";
my $view_isad_url = "http://www.archives.lib.ed.ac.uk/towardsdolly/cs/viewcat.pl";
...
print "<title>Catalogue - search results</title>";
...
print "<h2 class=\"main_head\">Search results</h2>";
...
if ((scalar @terms) > 5) {
  ...
  if ($total_results == 0) {
    ...
    print "<a href=\"searchcat.pl?term=$termstring&mode=$mode&results=$total_results\">$total_results</a>.</p>\n";

    @sorted_results = sort datecmp @results;
  
  ##  print "<ol start=\"$startpoint\">\n";
	print "<table summary=\"search results\" width=\"100%\">";
    for (my $count = $startpoint; $count <= $endpoint; $count += 1) {
      my $result = $sorted_results[$count - 1];
      my $titletext = @{$idtable{$result}}[1];
      print "<tr><td valign=\"top\">$count</td><td valign=\"top\" width=\"65%\"><a href=\"$view_isad_url?id=$result&view=basic\">$titletext</a> ";
      my $lowdate = @{$idtable{$result}}[2];
      my $highdate = @{$idtable{$result}}[3];
						my $trail = $idtable{$result}[0];
						my @ancestors = split /\//, $trail;
  					pop @ancestors;
						my $top_id = $ancestors[0];
      print "($lowdate";
      if (($lowdate =~ /\D/) || ($highdate != $lowdate)) {
        print "-$highdate";
      }
      print ")</td><td valign=\"top\">$idtable{$top_id}[1]</td></tr>\n";
    }
	print "</table>";
 ##   print "</ol>\n";
    ...
    if ($startpoint > 1 ) {
      ...
      print "<a href=\"searchcat.pl?term=$termstring&mode=$mode&start=$new_startpoint&results=$num_results\">Previous</a>\n";
    }

    if ($endpoint < $total_results) {
      my $new_startpoint = $endpoint + 1;
      print "<a href=\"searchcat.pl?term=$termstring&mode=$mode&start=$new_startpoint&results=$num_results\">Next</a>\n";
    }
