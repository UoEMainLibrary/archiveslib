...
$sql_str="SELECT DISTINCT ref, refid FROM images WHERE collection=97";
		$search = mysql_db_query($dbname, $sql_str, $id_link);
		echo "<ul>";
		while ($results = mysql_fetch_array($search)) {
		echo "<li><a href='cw.php?view=group&amp;refid=".$results['refid']."'>".$results['ref']."</a></li>";
		}
		echo "</ul>";
		echo "<p><a href='cw.php?view=all'>View all</a></p>";
...
if ($view == "all") {
	...
	$sql_str="SELECT * FROM images WHERE collection=97";
	$search = mysql_db_query($dbname, $sql_str, $id_link);
...
echo "</td></tr>";
echo "<tr><td><b>EAD code for this image</b></td><td align='right'>[<a href='cw.php?view=ead&amp;refid=".$results['refid']."'>Full <b>EAD code</b> for this ref</a>]</td></tr>";
...
echo "<tr><td colspan='2' align='right'><b><a href='cw.php?view=edit&amp;id=".$results['id']."'>edit metadata</a></b><hr /></td></tr>";
	}
	echo "</table>";
