...
/*=================================================
	...
	File: userexport.php
	Author:
		Boudewijn Sjouke	sjouke@users.sourceforge.net
	Comments:
		Exports users and their data to either SQL queries (Index mode) or authenticate.php and xxxxxx.dat files (MySQL mode).

	Change Log:
		08/04/2004 - File Created
...
if ($proceed == "backup") {
	print "<div class=\"center\"><h2>".$pgv_lang["um_backup"]."</h2>";
	print $pgv_lang["um_bu_explain"]."<br /><br />";
	...
	if ((!isset($_POST["um_config"])) && (!isset($_POST["um_gedcoms"])) && (!isset($_POST["um_gedsets"])) &&(!isset($_POST["um_logs"])) &&(!isset($_POST["um_usinfo"]))) {
		...
		<table align="center">
			<tr class="label"><?php print $pgv_lang["options"]; ?></tr><br />
			<tr><td><input type="checkbox" name="um_config" value="yes" checked="checked" /></td><td  style="text-align:<?php if ($TEXT_DIRECTION == "ltr") print "left"; else print "right";?>; "><?php print $pgv_lang["um_bu_config"]; ?></td></tr>
			<tr><td><input type="checkbox" name="um_gedcoms" value="yes" checked="checked" /></td><td  style="text-align:<?php if ($TEXT_DIRECTION == "ltr") print "left"; else print "right";?>; "><?php print $pgv_lang["um_bu_gedcoms"]; ?></td></tr>
			<tr><td><input type="checkbox" name="um_gedsets" value="yes" checked="checked" /></td><td  style="text-align:<?php if ($TEXT_DIRECTION == "ltr") print "left"; else print "right";?>; "><?php print $pgv_lang["um_bu_gedsets"]; ?></td></tr>
			<tr><td><input type="checkbox" name="um_logs" value="yes" checked="checked" /></td><td  style="text-align:<?php if ($TEXT_DIRECTION == "ltr") print "left"; else print "right";?>; "><?php print $pgv_lang["um_bu_logs"]; ?></td></tr>
			<tr><td><input type="checkbox" name="um_usinfo" value="yes" checked="checked" /></td><td  style="text-align:<?php if ($TEXT_DIRECTION == "ltr") print "left"; else print "right";?>; "><?php print $pgv_lang["um_bu_usinfo"]; ?></td></tr>
			</table><br /><br />
			<button type="submit" name="submit"><?php print $pgv_lang["um_mk_bu"]; ?></button>
	...
	if (isset($_POST["um_usinfo"])) {
		...
		if (($PGV_DATABASE == "db") && ($AUTHENTICATION_MODULE == "authentication_mysql.php")) {
	...
	if (isset($_POST["um_logs"])) {
		foreach($GEDCOMS as $key => $gedcom) {
			...
			while ($file = readdir ($dir_var)) {
				if ((strpos($file, ".log") > 0) && (strstr($file, "srch-".$gedcom["gedcom"]) !== false )) $flist[] = $INDEX_DIRECTORY.$file;
			}
			closedir($dir_var);
		}
	...
	if (count($flist) > 0) {
		...
		if ($v_list == 0) print "Error : ".$archive->errorInfo(true);
			...
			print $pgv_lang["um_zip_succ"]."<br /><br />";
			$url = $SERVER_URL;
			if (substr($url,-1,1)!="/") $url .= "/";
			print "<a href=".$url."downloadbackup.php?fname=".$buname." target=_blank>".$pgv_lang["um_zip_dl"]." ".$fname."</a>  (";
			printf("%.0f Kb", (filesize($fname)/1024));
			print")<br /><br />";
		}
		if (($PGV_DATABASE == "db") && ($AUTHENTICATION_MODULE == "authentication_mysql.php") && (isset($_POST["um_usinfo"]))) {
	...
	else print $pgv_lang["um_nofiles"];
...
// Check first if authentication mode is set to mysql while database is set to DB. If not, PGV already stores 
// the data in .dat files and migration is not necessary.
if (($PGV_DATABASE == "db") && ($AUTHENTICATION_MODULE == "authentication_index.php")) {
	print "\n\t<h2>".$pgv_lang["um_header"]."</h2>";
	print "<br /><br />".$pgv_lang["um_mixedmode"];
	print  "<br /><br /><a href=\"admin.php\">" . $pgv_lang["lang_back_admin"] . "</a><br /><br />";
	print_footer();
	exit;
}

// Check if mode is index. No action is required, as the import will be done from SQL mode.
if ($PGV_DATABASE == "index") {
	print "\n\t<h2>".$pgv_lang["um_header"]."</h2>";
	print "<br /><br />".$pgv_lang["um_noaction"];
	print  "<br /><br /><a href=\"admin.php\">" . $pgv_lang["lang_back_admin"] . "</a><br /><br />";
	print_footer();
	exit;
}
...
if (($proceed != "import") && ($proceed != "export") && ($proceed != "exportovr")) {
	...
	<form action="usermigrate.php" method="post">
			<input type="hidden" name="proceed" value="import" />
			<?php print $pgv_lang["um_explain"].$pgv_lang["um_proceed"]?> <br />
			<input type="submit" class="button" value="<?php print $pgv_lang["um_import"];?>" />
...
function um_export() {
	...
	foreach($users as $key=>$user) {
		$user["fullname"] = $DBCONN->escapeSimple($user["fullname"]);
		$authtext .= "\$user = array();\n";
...
if (($proceed == "export") || ($proceed == "exportovr")) {
	...
	if ($proceed == "export") {
		...
		if ($i > 0) {
			...
			?>
			<form "<?php print $PHP_SELF; ?>" method="post">
