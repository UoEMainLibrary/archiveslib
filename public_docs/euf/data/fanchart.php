 ...
 * @package PhpGedView
 * @subpackage Charts
 * @version $Id: fanchart.php,v 1.25 2005/05/06 14:43:43 yalnifj Exp $
 */
require("config.php");
...
function split_align_text($data, $maxlen) {
	...
	if ($found) $line=$data;
//	if (strpos($data, chr(215)) !== false || strpos($data, chr(216)) !== false || strpos($data, chr(217)) !== false || strpos($data, chr(218)) !== false || strpos($data, chr(219)) !== false) $line=$data;
	else
...
//		if (ord($line{0})==215 || ord($line{0})==216 || ord($line{0})==217 || ord($line{0})==218 || ord($line{0})==219) $len/=2; // hebrew text
		$p = max(0,floor(($maxlen-$len)/2));
		$line = str_repeat(" ", $p) . "$line"; // center alignment using spaces
		$text .= "$line";
	}
...
if ($view != "preview") {
	...
	print "</tr><tr>";
	// fan width
	print "<td class=\"list_label\">&nbsp;" . $pgv_lang["fan_width"] . "&nbsp;</td>";
	print "<td class=\"list_value\">";
