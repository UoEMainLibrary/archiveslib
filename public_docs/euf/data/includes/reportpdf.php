...
if (strstr($_SERVER["PHP_SELF"],"reportpdf.php")) {
	...
	if (!empty($tag)) {
		...
		if ($tag=="@id") {
			...
			if ($ct > 0) {
				...
				if ($t=="DATE") $value = get_changed_date($value);
...
function PGVRFactsSHandler($attrs) {
	...
	$repeats = get_all_subrecords($gedrec, $tag);
}
...
//	$ct = preg_match("/@([A-Za-z0-9]+)/", $condition, $match);
...
function PGVRRelativesSHandler($attrs) {
	...
	function add_ancestors($pid, $children=false) {
		...
		while(count($genlist)>0) {
			...
			if (count($famids)>0) {
				foreach($famids as $indexval => $famid) {
					...
					if (!empty($parents["HUSB"])) array_push($genlist, $parents["WIFE"]);
					if ($children) {
	...
	function add_descendancy($pid, $parents=false) {
			...
			foreach($famids as $indexval => $famid) {
				...
				if ($famrec) {
					...
					for($i=0; $i<$num; $i++) {
						find_person_record($smatch[$i][1]);
						$list[$smatch[$i][1]] = $indilist[$smatch[$i][1]];
						if (isset($list[$smatch[$i][1]]["generation"])) $list[$smatch[$smatch[$i][1]][1]]["generation"] = $list[$id]["generation"]+1;
						else $list[$smatch[$i][1]]["generation"] = 2;
					}
					for($i=0; $i<$num; $i++) {
						add_descendancy($smatch[$i][1], $parents);	// recurse on the childs family
					}
				}
			}
		}
