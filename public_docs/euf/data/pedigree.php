 ...
 *
 * $Id: pedigree.php,v 1.55 2005/11/09 19:00:18 yalnifj Exp $
 * @package PhpGedView
 * @subpackage Charts
 */
require("config.php");
...
print "<div style=\"position: relative; z-index: 1; width: 250px;\">\n";
...
if ($view!="preview") {
	...
	print "&nbsp;</td><td>";
	print "<input type=\"text\" name=\"PEDIGREE_GENERATIONS\" size=\"1\" value=\"$OLD_PGENS\" />";
	print_help_link("PEDIGREE_GENERATIONS_help", "qm");
	print "</td></tr><tr><td class=\"news_date\" colspan=\"2\">";
	...
	print "<input type=\"radio\" name=\"talloffset\" value=\"1\"";
	if ($talloffset) print "checked=\"checked\" ";
	...
	print "\n\t\t<br /><input type=\"submit\" value=\"".$pgv_lang["view"]."\" />";
	print "</td></tr>";
	print "</table>";
	print "\n\t\t</form>";
...
for($i=($treesize-1); $i>=0; $i--) {
	...
	if (!empty($treeid[$i]) || $SHOW_EMPTY_BOXES) {
		...
		if ($TEXT_DIRECTION=="rtl") print ".0\" style=\"position:absolute; right:";
		else print ".0\" style=\"position:absolute; left:";
...
if (displayDetails($indirec) || showLivingName($indirec)) {
	...
	if ($famids||($num>1)) {
		...
		if ($view!="preview") {
			if ($TEXT_DIRECTION=="rtl") print "<a href=\"#\" onclick=\"return togglechildrenbox();\" onmouseover=\"swap_image('larrow',1);\" onmouseout=\"swap_image('larrow',1);\">";
			else print "<a href=\"#\" onclick=\"return togglechildrenbox();\" onmouseover=\"swap_image('larrow',0);\" onmouseout=\"swap_image('larrow',0);\">";
