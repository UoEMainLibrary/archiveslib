...
foreach ($subj_arr as $authfilenumber) {
 ...
 $sql_str="SELECT * FROM cms_auth_subj where id='$authfilenumber' AND id <> ''"; 
 ...
 if ($authfilenumber <>'') {
	...
	echo $count_str." ".$term." ".$authfilenumber."<br />";
...
foreach ($geog_arr as $authfilenumber) {
 ...
 if ($authfilenumber <>'') {
	...
	fwrite ($handle, "<country>".$results['country']);
		...
		## if county is set and city is not set and island is not set and area is not set
		elseif ($results['county'] <>'' && $results['city'] =='' && $results['island'] =='' && $results['term'] =='') {
				fwrite ($handle, "<county>".$results['county']."</county>\n");
		}
...
$sub_genr = file_get_contents("sub_genr.txt");

$genr_arr_p = explode (",", $sub_genr);

sort($genr_arr_p);

$genr_arr = array_unique($genr_arr_p);

$count_str = "0";

foreach ($genr_arr as $authfilenumber) {


 $sql_str="SELECT * FROM cms_auth_genr where id='$authfilenumber' AND id <> ''"; 
 $subjsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
 $results = mysql_fetch_array($subjsearch);
...
$sub_pers = file_get_contents("sub_pers.txt");

$pers_arr_p = explode (",", $sub_pers);

sort($pers_arr_p);

$pers_arr = array_unique($pers_arr_p);

$count_str = "0";

foreach ($pers_arr as $authfilenumber) {


 $sql_str="SELECT * FROM cms_auth_pers where id='$authfilenumber' AND id <> ''"; 
 $perssearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
 $results = mysql_fetch_array($perssearch);
 
 if ($authfilenumber <>'') {
	fwrite ($handle,  "<mads>\n<authority>\n");
	$term = htmlspecialchars(($results['given_name']." ".$results['family_name']), ENT_QUOTES);
...
$sub_corp = file_get_contents("sub_corp.txt");

$corp_arr_p = explode (",", $sub_corp);

sort($corp_arr_p);

$corp_arr = array_unique($corp_arr_p);

$count_str = "0";

foreach ($corp_arr as $authfilenumber) {


 $sql_str="SELECT * FROM cms_auth_corp where id='$authfilenumber' AND id <> ''"; 
 $corpsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
 $results = mysql_fetch_array($corpsearch);
 
 if ($authfilenumber <>'') {
	fwrite ($handle,  "<mads>\n<authority>\n");
	$term = htmlspecialchars(($results['primary_name']." ".$results['secondary_name']), ENT_QUOTES);
...
$sub_fam = file_get_contents("sub_fam.txt");

$fam_arr_p = explode (",", $sub_fam);

sort($fam_arr_p);

$fam_arr = array_unique($fam_arr_p);

$count_str = "0";

foreach ($fam_arr as $authfilenumber) {


 $sql_str="SELECT * FROM cms_auth_fam where id='$authfilenumber' AND id <> ''"; 
 $famsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
 $results = mysql_fetch_array($famsearch);
 
 if ($authfilenumber <>'') {
	fwrite ($handle,  "<mads>\n<authority>\n");
	$term = htmlspecialchars(($results['family_name']." ".$results['territorial_distinction']), ENT_QUOTES);
	...
	$use_for = htmlspecialchars(($results['use_for']), ENT_QUOTES);
	
	fwrite ($handle, "<name authority=\"".$id."\" type=\"personal\">\n");
	fwrite ($handle, "<namePart type=\"family\">".$family_name."</namePart>\n");
	if ($title <>''){
	fwrite ($handle, "<namePart type=\"termsOfAddress\">".$title."</namePart>\n");
		}
