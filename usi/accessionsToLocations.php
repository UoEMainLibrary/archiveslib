...
$number = $_GET['number'];
	 ...
	 echo "<h3>CLX boxes and unboxed ('E' accessions)</h3>";
	 
	 if (!isset($view)) { 
echo "<form method='GET' action='".$PHP_SELF."'>";
echo "<input type='hidden' name='view' value='interim' />";	
echo "<input type='hidden' name='func' value='accessionsToLocations' />";	
...
$sql_str="SELECT distinct number FROM cms_clx WHERE year LIKE '$year' AND number>0 ORDER BY number ASC";
$boxsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
while ($results = mysql_fetch_array($boxsearch)):	
...
echo "</form";
	 ...
	 # old shelfmarks first
	 
	  $sql_str="SELECT * FROM cms_accessionsToShelfmarks WHERE year like '$year' and number LIKE '$number'";
	  
	  $smsearch = mysql_db_query($dbname, $sql_str, $id_link) or die("Search Failed!");
	  
	  echo "<table cellpadding='5'>";
	  echo "<tr><td>Shelfmark</td></tr>";
	  
	  while ($results = mysql_fetch_array($smsearch)):
	  
	  echo "<tr><td>".$results['sm_prefix'].".".$results['sm_1'].".".$results['sm_2']." ".$results['sm_suffix']."</td><td>".$results['sm_notes']."</td><td><i>(written as ".$results['sm_written'].")</i></td></tr>";
